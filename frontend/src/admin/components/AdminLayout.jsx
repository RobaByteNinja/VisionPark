...
import {
    ...
    Moon, Sun, PanelLeft, ShieldAlert
} from "lucide-react";
...
const SidebarContent = ({ collapsed, currentPath, onNavigate, onHover, onLeave }) => (
    <div className="flex flex-col h-full bg-white dark:bg-[#18181b] w-full">
        ...
        <div className={`flex flex-col items-center border-b border-zinc-200 dark:border-white/5 shrink-0 transition-all duration-300 ${collapsed ? 'py-4' : 'py-6'}`}>
            {/* Avatar — initials based since admin has no photo on first load */}
            <div className={`rounded-full border-2 border-indigo-500 shadow-sm bg-indigo-600 flex items-center justify-center text-white font-bold transition-all duration-300 ${collapsed ? 'h-10 w-10 text-xs border' : 'h-16 w-16 text-lg'}`}>
                {MOCK_ADMIN.initials}
            </div>
            <div className={`flex flex-col items-center overflow-hidden whitespace-nowrap transition-all duration-300 ${collapsed ? 'opacity-0 max-h-0 mt-0' : 'opacity-100 max-h-[100px] mt-3'}`}>
                <h3 className="font-bold text-zinc-900 dark:text-white">{MOCK_ADMIN.name}</h3>
                <span className="text-xs text-indigo-500 font-bold uppercase tracking-wider">{MOCK_ADMIN.role}</span>
            </div>
        </div>
                    ...
                    <div className="space-y-1">
                        {group.items.map((item) => {
                            ...
                            return (
                                ...
                                >
                                    <Icon className={`h-4 w-4 shrink-0 transition-colors ${isActive ? "text-indigo-400 dark:text-indigo-400" : "text-zinc-400 dark:text-zinc-500 group-hover:text-white"}`} />
...
export default function AdminLayout() {
    const [isSidebarCollapsed, setIsSidebarCollapsed] = useState(() => {
        const saved = localStorage.getItem("visionpark_admin_sidebar_collapsed");
        return saved === "true";
    });
    ...
    const handleLogout = () => {
        setIsProfileDropdownOpen(false);
        // 🛑 TEAMS: clear token, then navigate 🛑
    ...
    return (
        <div className="flex h-[100dvh] w-full overflow-hidden bg-[#f4f4f5] dark:bg-[#09090b] transition-colors duration-500">
            ...
            <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden relative">
                ...
                {/* Top Header */}
                <header className="h-16 lg:h-20 bg-white/95 dark:bg-[#18181b]/95 backdrop-blur-md border-b border-zinc-200 dark:border-white/5 flex items-center justify-between px-4 lg:px-8 z-40 shrink-0 transition-colors duration-500">
                    ...
                    <div className="flex items-center gap-2 md:gap-4 relative">
                        ...
                        <div className="relative">
                            ...
                            >
                                <div className="h-8 w-8 md:h-9 md:w-9 rounded-full bg-indigo-600 flex items-center justify-center text-white text-xs font-bold border border-indigo-500/30 shadow-sm shrink-0">
                                    {MOCK_ADMIN.initials}
                                </div>
                                <span className="hidden md:block text-sm font-bold text-zinc-900 dark:text-white">{MOCK_ADMIN.name}</span>
                            ...
                            {isProfileDropdownOpen && (
                                <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-[#18181b] rounded-2xl shadow-xl border border-zinc-200 dark:border-white/10 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 z-[5000]">
                                    <div className="px-4 py-3 border-b border-zinc-100 dark:border-white/5">
                                        <p className="text-sm font-bold text-zinc-900 dark:text-white">{MOCK_ADMIN.name}</p>
                                        <p className="text-xs text-indigo-400 font-bold uppercase tracking-wider">{MOCK_ADMIN.role}</p>
                                    </div>

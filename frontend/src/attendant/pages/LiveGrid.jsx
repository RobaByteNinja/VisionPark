...
const generateMockGrid = () => {
    ...
    rows.forEach(row => {
        for (let i = 1; i <= 10; i++) {
            ...
            if (id === 'A01') {
                // SCENARIO: Conflict (Unauthorized Squatter in Reserved Spot)
                status = "Conflict";
                isConflict = true;
                expectedDriver = { name: "Dawit M.", eta: "8 mins", plate: "AA 12345" };
                actualPlate = "UNKNOWN"; // Camera sees a car, but it's not Dawit
            }
            else if (id === 'A02') {
                // SCENARIO: Another Conflict
                status = "Conflict";
                isConflict = true;
                expectedDriver = { name: "Sara T.", eta: "15 mins", plate: "OR 99887" };
                actualPlate = "AA 45678";
            }
            // RANDOM FILL FOR THE REST (A03 is now just a normal spot)
            else {
        ...
        <div className="h-full w-full flex gap-6 animate-in fade-in duration-500 relative overflow-hidden">
            ...
            {selectedSpot && (
                <div className="w-[360px] lg:w-[420px] h-full bg-white dark:bg-[#121214] rounded-3xl shadow-2xl border border-zinc-200 dark:border-white/5 flex flex-col shrink-0 animate-in slide-in-from-right-8 duration-300 relative">
                    ...
                    {/* Premium Green Toast */}
                    {toastMessage && (
                        <div className="absolute top-20 left-4 right-4 bg-emerald-500 text-emerald-950 font-bold text-sm px-4 py-3 rounded-xl shadow-lg z-50 animate-in fade-in slide-in-from-top-4 flex items-center gap-2">
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                                ...
                                <div className="w-full h-full relative">
                                    ...
                                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                        <div className={`w-2/3 h-1/2 border-[3px] rounded-lg relative ${selectedSpot.isConflict ? 'border-purple-500/80' : 'border-emerald-500/50'}`}>
                                            <span className={`absolute -top-6 left-0 text-black text-[10px] font-bold px-1.5 py-0.5 rounded-t-md ${selectedSpot.isConflict ? 'bg-purple-500 text-white' : 'bg-emerald-500'}`}>
                            ...
                            <div className="bg-purple-50 dark:bg-purple-500/10 border border-purple-200 dark:border-purple-500/20 rounded-2xl p-5 animate-in fade-in zoom-in duration-300">
                                ...
                                {selectedSpot.waitingToMove ? (
                                    ...
                                    <div className="space-y-3">
                                        <p className="text-xs text-purple-900 dark:text-purple-300 font-medium">Unauthorized vehicle in reserved spot. Instruct driver to leave.</p>
                                        {/* ✅ Premium Red for Turn Away / Leave Action */}
                        ...
                        {/* REGISTERED USER DATA (For valid Reservations or Occupied spots) */}
                        {selectedSpot.expectedDriver && !selectedSpot.isConflict && (
                            <div className="space-y-4 animate-in fade-in duration-300">
                                <h4 className="text-xs font-black uppercase tracking-widest text-zinc-400 border-b border-zinc-100 dark:border-white/5 pb-2">User & Vehicle Data</h4>
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-wider">Driver</p>

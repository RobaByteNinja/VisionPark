...
export default function Shift() {
    ...
    // 'pre-shift' | 'active' | 'closing' | 'completed'
    ...
    const handleSignOut = () => {
        // Ejects the user from the app to the login screen
        navigate("/");
    };

    const handleCancelZReport = () => {
        // Allows them to close the Z-Report and go back to checking the lot
        setShiftState("active");
        setActualCashInput("");
        setZReport(null);
    };
        ...
        return (
            <div className="h-full w-full flex items-center justify-center p-6 animate-in fade-in duration-500">
                <div className="max-w-md w-full bg-white dark:bg-[#121214] rounded-3xl shadow-xl border border-zinc-200 dark:border-white/5 p-8 text-center flex flex-col items-center">
                    ...
                    {/* ✅ Premium Green for Create/Start Action */}
                    <button
    ...
    // ✅ THE THEME-ADAPTIVE Z-REPORT VIEW
    if (shiftState === "completed" && zReport) {
        const isExact = zReport.variance === 0;
        const isShort = zReport.variance < 0;

        return (
            <div className="h-full w-full flex items-center justify-center p-6 animate-in zoom-in-95 duration-500 overflow-y-auto custom-scrollbar">
                <div className="max-w-md w-full bg-white dark:bg-[#18181b] rounded-2xl shadow-2xl flex flex-col border border-transparent dark:border-white/10 overflow-hidden">
    ...
    return (
        <div className="h-full w-full flex flex-col xl:flex-row gap-6 animate-in fade-in duration-500 relative">
                ...
                <div className="flex-1 bg-white dark:bg-[#121214] rounded-3xl shadow-sm border border-zinc-200 dark:border-white/5 p-6 md:p-8 flex flex-col relative overflow-hidden">
                    ...
                    {shiftState === "active" ? (
                        <div className="space-y-8 flex-1 animate-in fade-in zoom-in-95 duration-300">
                            ...
                            {/* Digital Summary (Does not show actual cash total for blind close) */}
                            ...
                            <div className="mt-auto pt-8">
                                {/* ✅ Blue for Continue / Generate Action */}
                                <button
                        ...
                        <div className="space-y-6 flex-1 animate-in slide-in-from-right-8 duration-300 flex flex-col justify-center max-w-sm mx-auto w-full">
                            ...
                            <div className="flex gap-3 pt-4">
                                ...
                                {/* ✅ Premium Green for Submit / Confirm Action */}
                                <button
                                ...
                                >
                                    {isSubmitting ? <span className="animate-pulse">Generating Z-Report...</span> : <>Submit & Close <CheckCircle className="h-5 w-5" /></>}
                                </button>
                            </div>
            ...
            {/* RIGHT COLUMN: Recent Cash Activity Log */}

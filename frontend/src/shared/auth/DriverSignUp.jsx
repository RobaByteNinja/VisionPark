...
export default function DriverSignUp() {
  ...
  // ✅ SMART CONTEXT-AWARE EMAIL VALIDATION
  useEffect(() => {
    ...
    if (email.length > 0) {
      ...
      if (localPart.length === 0) {
        setEmailError("Please enter the part before the '@'.");
        return;
      }

      if (!domainPart || domainPart.length === 0) {
        setEmailError("Please enter a domain after the '@'.");
        return;
      }

      if (!domainPart.includes('.')) {
        setEmailError(`Please complete the domain extension (e.g., ${domainPart}.edu.et, ${domainPart}.com).`);
        return;
      }
      ...
      if (tld.length < 2) {
        setEmailError("Domain extension is too short.");
        return;
      }
      ...
      if (typos[domainPart.toLowerCase()]) {
        setEmailError(`Did you mean @${typos[domainPart.toLowerCase()]}?`);
        return;
      }
      ...
      {activeModal && (
        <div className="fixed inset-0 z-[6000] flex items-center justify-center bg-zinc-900/40 dark:bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="w-full max-w-sm md:max-w-md bg-white dark:bg-[#121214] rounded-3xl shadow-2xl flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-300" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="overflow-y-auto p-3 md:p-4 overscroll-contain">
                ...
                <div key={gIndex} className="mb-4 last:mb-0">
                  ...
                  <div className="space-y-1 md:space-y-2">
                    {group.items.map((item, iIndex) => {
                      ...
                      return (
                        <button key={iIndex} onClick={() => handleSelectOption(activeModal, item)} className={`w-full flex items-center justify-between p-4 rounded-xl text-left transition-all outline-none cursor-pointer active:scale-[0.98] ${isSelected ? 'bg-emerald-50 dark:bg-emerald-500/10' : 'hover:bg-zinc-50 dark:hover:bg-white/5 active:bg-zinc-100 dark:active:bg-zinc-800'}`}>
